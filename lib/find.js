...
let cmakePaths = cmakePath.split(path.delimiter);
...
module.exports = function (messagePackage) {
  ...
  const found = cmakePaths.some((cmakePath) => {
    let indexPath = path.join(cmakePath, jsMsgPath, messagePackage, '_index.js');
    try {
      packagePaths[messagePackage] = require(indexPath);
    }
    catch (err) {
      return false;
    }
